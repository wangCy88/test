...
class IndexController extends Controller
{
    ...
    public static function changeModel(Request $request){
        $id = $request->id;
        if(!empty($id)){
            $model = AuthRoutes::select('name')->find($id)->name;
            $gid = session('user_info')->gid;
            $data = AuthRoutes::where(['upid' => $id]);
            if($gid !== 0){
                $routes = AuthGroups::select('routes')->find($gid)->routes;
                $routes = explode(',', $routes);
                $data = $data->whereIn('id' ,$routes);
            ...
            return response()->json(['code' => 0, 'data' => $data, 'model' => $model, 'url' => 'https://cv.liangziloan.com/']);
        }else{
    ...
    public static function login(){
        if($_POST){
            ...
            $data = DB::table('auth_users')->where(['account' => $username, 'password' => md5($password)])->first();
            if(!$data){
